...
# from models import Post, TwitterFeedProcessor
from request import TwitterFeedProcessor
...
NG_FILE_LOCATION = (
    "/home/rockwell/Rockwell/backend/src/recsys/NewsGuardIffy/label-2022101916.json"
)
...
access_token_store = SqliteDict(
    db_name, tablename="access_token_store ", autocommit=True
)
access_token_secret_store = SqliteDict(
    db_name, tablename="access_token_secret_store", autocommit=True
)
...
twitterversion_store = SqliteDict(
    db_name, tablename="twitterversion_store", autocommit=True
)
mode_store = SqliteDict(db_name, tablename="mode_store", autocommit=True)
participant_id_store = SqliteDict(
    db_name, tablename="participant_id_store", autocommit=True
)
assignment_id_store = SqliteDict(
    db_name, tablename="assignment_id_store", autocommit=True
)
project_id_store = SqliteDict(db_name, tablename="project_id_store", autocommit=True)
hometimeline_pulled_store = SqliteDict(
    db_name, tablename="hometimeline_pulled_store", autocommit=True
)
completed_survey = SqliteDict(db_name, tablename="completed_survey", autocommit=True)
experimental_condition = SqliteDict(
    db_name, tablename="experimental_condition", autocommit=True
)
    ...
    for i, tweet in enumerate(feedtweetsv1):
        ...
        if tweet["id_str"] not in level_1_tweets:
            if "retweeted_status" in tweet.keys():
                if (
                    tweet["retweeted_status"]["id_str"] not in level_1_tweets
                    and tweet["retweeted_status"]["id_str"] not in level_2_retweeted
                ):
...
def break_timeline_attention(
    public_tweets, public_tweets_score, absent_tweets, max_pages
):
    db_tweet_payload = []
    ...
    for attn_page in range(max_pages):
        ...
        present_tweets_select = np.random.choice(
            present_tweets_ids, size=3, replace=False
        )
        absent_tweets_select = np.random.choice(
            absent_tweets_ids, size=2, replace=False
        )
        for absent_tweet_id in absent_tweets_select:
            absent_tweets_ids.remove(absent_tweet_id)
        all_attn_tweets = np.concatenate(
            (present_tweets_select, absent_tweets_select), axis=0
        )
...
def addallfields(v2tweet, v2user, v2media, v2tweetobj_user=None, v2tweetobj_fav=None):
    ...
    v1tweet["user"]["profile_image_url"] = v2user[v2tweet["author_id"]][
        "profile_image_url"
    ]
...
def convertv2tov1(v2tweetobj, cred, v2tweetobj_user=None, v2tweetobj_fav=None):
    ...
    tweet_1_lookup, tweet_1_media_lookup, tweet_1_user_lookup, tweet_2_ids = (
        createlookups(v2tweetobj)
    )
    ...
    if tweet_2_ids:
        ...
        response_tweet_2 = oauth_new.get(
            "https://api.twitter.com/2/tweets", params=new_tweet_params
        )
        v2tweetobj_2 = json.loads(response_tweet_2.text)
        tweet_2_lookup, tweet_2_media_lookup, tweet_2_user_lookup, tweet_3_ids = (
            createlookups(v2tweetobj_2, onlyincludequote=True)
        )
    ...
    if tweet_3_ids:
        ...
        response_tweet_3 = oauth_new.get(
            "https://api.twitter.com/2/tweets", params=new_tweet_params
        )
        v2tweetobj_3 = json.loads(response_tweet_3.text)
        tweet_3_lookup, tweet_3_media_lookup, tweet_3_user_lookup, no_matter_ids = (
            createlookups(v2tweetobj_3, includenext=False)
        )

    if "data" in v2tweetobj.keys():
        for v2tweet in v2tweetobj["data"]:
            v1tweet = addallfields(
                v2tweet,
                tweet_1_user_lookup,
                tweet_1_media_lookup,
                v2tweetobj_user=v2tweetobj_user,
                v2tweetobj_fav=v2tweetobj_fav,
            )
            if "referenced_tweets" in v2tweet.keys():
                for referenced_tweet in v2tweet["referenced_tweets"]:
                    if referenced_tweet["type"] == "retweeted":
                        if referenced_tweet["id"] in tweet_2_lookup.keys():
                            ...
                            v1tweet["retweeted_status"] = addallfields(
                                v2tweet_retweeted,
                                tweet_2_user_lookup,
                                tweet_2_media_lookup,
                            )
                            if "referenced_tweets" in v2tweet_retweeted.keys():
                                for double_referenced_tweet in v2tweet_retweeted[
                                    "referenced_tweets"
                                ]:
                                    if double_referenced_tweet["type"] == "quoted":
                                        if (
                                            double_referenced_tweet["id"]
                                            in tweet_3_lookup.keys()
                                        ):
                                            v2tweet_retweeted_quoted = tweet_3_lookup[
                                                double_referenced_tweet["id"]
                                            ]
                                            v1tweet["retweeted_status"][
                                                "quoted_status"
                                            ] = addallfields(
                                                v2tweet_retweeted_quoted,
                                                tweet_3_user_lookup,
                                                tweet_3_media_lookup,
                                            )
                    if referenced_tweet["type"] == "quoted":
                        if referenced_tweet["id"] in tweet_2_lookup.keys():
                            ...
                            v1tweet["quoted_status"] = addallfields(
                                v2tweet_quoted,
                                tweet_2_user_lookup,
                                tweet_2_media_lookup,
                            )
...
def start():
    ...
    try:
        request_token = OAuth1Session(
            client_key=cred["key"], client_secret=cred["key_secret"]
        )
        content = request_token.post(
            request_token_url, data={"oauth_callback": app_callback_url}
        )
    ...
    res = make_response(
        render_template(
            "YouGov.html", start_url=start_url, screenname="###", rockwell_url="###"
        )
    )
...
def qualstart():
    ...
    try:
        ...
        content = request_token.post(
            request_token_url, data={"oauth_callback": app_callback_url_qual}
        )
        logging.info("Twitter access successfull")
    except Exception as error:
        print("Twitter access failed with error : " + str(error))
...
def logerrors():
    ...
    logging.info(
        f"Error in the frontend : {worker_id=} {ip_address=} {message=} {stack=}"
    )
    return "Done!"
...
def qualrender():
    ...
    res = make_response(
        render_template(
            "YouGovQualtrics.html",
            start="Yes",
            start_url=start_url,
            oauth_token=oauth_token_qualtrics,
            mode=mode,
...
def qualcallback():
    ...
    cred = config("../configuration/config.ini", "twitterapp")
    oauth_access_tokens = OAuth1Session(
        client_key=cred["key"],
        client_secret=cred["key_secret"],
    ...
    oauth_account_settings = OAuth1Session(
        client_key=cred["key"],
        client_secret=cred["key_secret"],
    ...
    response_creation_date = oauth_account_settings.get(
        creation_date_url, params={"ids": [user_id]}
    )
    ...
    if mode == "ELIGIBILITY":
        ...
        db_response_worker_id = requests.get(
            "http://127.0.0.1:5052/get_existing_user_by_twitter_id?twitter_id="
            + str(user_id)
        )
        if db_response_worker_id.json()["data"] != "NEW":
            ...
            insert_user_payload = {
                ...
                "account_settings": account_settings_user,
                "creation_date": creation_date_user,
                "oauth_token": oauth_token,
                "participant_id": participant_id,
            }
            requests.get(
                "http://" + webInformation["localhost"] + ":5052/insert_user",
        ...
        db_response_screenname = requests.get(
            "http://127.0.0.1:5052/get_existing_tweets_new_screenname?screenname="
            + str(screen_name)
            + "&page="
            + str(0)
            + "&feedtype=S"
...
def insert_feed_qualtrics():
    ...
    db_response = requests.get(
        "http://127.0.0.1:5052/get_existing_user?worker_id=" + str(worker_id)
    )
    ...
    print(screenname)
    db_response = requests.get(
        "http://127.0.0.1:5052/get_existing_tweets_new?worker_id="
        + str(worker_id)
        + "&page="
        + str(0)
        + "&feedtype=S"
    ...
    if need_to_fetch_screenname:
        db_response_screenname = requests.get(
            "http://127.0.0.1:5052/get_existing_tweets_new_screenname_screen_2?screenname="
            + str(screenname)
            + "&page="
            + str(0)
            + "&feedtype=S"
...
def screenname():
    ...
    random_identifier = "".join(
        ...
        for _ in range(random_identifier_len)
    )
    ...
    file_number = 1
    existing_home_timeline_files = sorted(
        glob.glob("UserData/{}_home_*.json.gz".format(userid_return))
    )
    if existing_home_timeline_files:
        latest_user_file = max(
            existing_home_timeline_files,
            key=lambda fn: int(fn.split(".")[0].split("_")[2]),
        )
...
def get_hometimeline_from_file():
    ...
    for v2tweetobj in v2tweetobj_arr:
        ...
        feed_tweets_itr, feed_tweets_v2_itr = filter_tweets(
            v1tweetobj, v2tweetobj["data"]
        )
        ...
        for i, tweet in enumerate(feed_tweets_itr):
            ...
            db_tweet_payload.append(db_tweet)
    db_response = requests.get(
        "http://127.0.0.1:5052/get_existing_tweets_all?worker_id=" + str(worker_id)
    )
    if db_response.json()["data"] != "NEW":
    ...
    requests.post(
        "http://127.0.0.1:5052/insert_timelines_attention_chronological", json=finalJson
    )
    return "Done!"
...
def get_hometimeline():
    ...
    logging.info(
        f"Twitter information : {worker_id=} {access_token=} {access_token_secret=}"
    )
    v2tweetobj = {}
    v1tweetobj = {}
    ...
    for i in range(num_itr):
        ...
        response = oauth.get(
            "https://api.twitter.com/2/users/{}/timelines/reverse_chronological".format(
                userid
            ),
        ...
        if errormessage == "NA":
            ...
            v1tweetobj_arr.append(v1tweetobj)
            logging.info(
                f"Converted Version 2 Twitter JSON object to Version 1 : {worker_id=}"
            )
    ...
    userobj = {"screen_name": screenname, "twitter_id": userid}

    queries = compose_queries_512_chars(screenname)
    user_eng_queries = []
    for qq in queries:
        user_eng_queries.append(
            {"query": qq, "since_id": "0", "next_token": "##START##"}
        )
    ...
    with gzip.open(
        "hometimeline_data/{}_home_{}.json.gz".format(userid, file_number), "w"
    ) as outfile:
        outfile.write(json.dumps(writeObj).encode("utf-8"))

    with gzip.open(
        "UserDatav2/{}_home_{}.json.gz".format(userid, file_number), "w"
    ) as outfile:
    ...
    if db_tweet_payload:
        ...
        requests.post(
            "http://127.0.0.1:5052/insert_timelines_attention_chronological",
            json=finalJson,
        )
    ...
    for i in range(num_itr):
        ...
        if pagination_token:
            timeline_params_cap["pagination_token"] = pagination_token
        response = oauth.get(
            "https://api.twitter.com/2/users/{}/tweets".format(userid),
            params=timeline_params_cap,
        )
        if (
            response.text
    ...
    for i in range(num_itr):
        ...
        if pagination_token:
            timeline_params_cap["pagination_token"] = pagination_token
        response = oauth.get(
            "https://api.twitter.com/2/users/{}/liked_tweets".format(userid),
            params=timeline_params_cap,
        )
        if (
            response.text
...
def get_hometimeline_prev():
    ...
    screen_2_tweet_ids = []
    db_response_tweets_screen_2 = requests.get(
        "http://127.0.0.1:5052/get_tweets_screen_2"
    )
    if db_response_tweets_screen_2.json()["data"] != "NEW":
        screen_2_tweet_ids = [
            response[0] for response in db_response_tweets_screen_2.json()["data"]
        ]
    ...
    requests.post(
        "http://127.0.0.1:5052/insert_timelines_screen_2_not_control", json=finalJson
    )
    logging.info(f"Completed database insertion : {worker_id=}")
...
def get_usertimeline():
    ...
    response = oauth.get(
        "https://api.twitter.com/2/users/{}/tweets".format(userid),
        params=timeline_params_engagement,
    )
    if (
        response.text
        == '{"errors":[{"code":89,"message":"Invalid or expired token."}]}'
    ...
    if hoaxy_tweets:
        ...
        with gzip.open(
            "usertimeline_hoaxy_data/{}_user_hoaxy.json.gz".format(userid), "w"
        ) as outfile:
            outfile.write(json.dumps(writeObj).encode("utf-8"))
...
def get_feed():
    """API endpoint to get processed Twitter feed"""
    start_time = datetime.now()
    try:
        # Load tweets from file
        with open("/home/rivaldoe/Downloads/sample_posts_TWITTER.json", "r") as fp:
            tweets = json.load(fp)["data"]
            tweets = tweets[:5]
        
        # Process tweets into ContentItems
        ...
        for idx, tweet in enumerate(tweets):
            item = processor.process_tweet(tweet, idx).dict()  # Convert to dict
            # Explicitly handle non-serializable fields
            item["embedded_urls"] = [str(url) for url in item.get("embedded_urls", [])]  # Convert URLs to strings
            if "created_at" in item:
                item["created_at"] = item["created_at"].isoformat()  # Convert datetime to ISO format
            content_items.append(item)
        
        
        processing_time = (datetime.now() - start_time).total_seconds()
        print(f"Feed processing completed in {processing_time:.2f} seconds")
...
# # TODO: validate user, right now there is no user here
# @app.route("/getfeed", methods=["GET"])
# def get_feed():
...
#         # if the tweet does not pass pydantic validation this will crash the entire request
#         post = Post(**tweet)
#
...
#         domain_present = ""
#
#         if "entities" in post.__pydantic_extra__:
#             if "urls" in post.entities:
#                 for url_dict in post.entities["urls"]:
...
#                 full_text_json.append(
#                     {"text": full_text[normal_idx:url_idx_start], "url": ""}
#                 )
#                 full_text_json.append({"text": url_extend[i], "url": url_extend[i]})
#                 normal_idx = url_end[i]
#             if normal_idx < len(full_text):
#                 full_text_json.append(
#                     {"text": full_text[normal_idx : len(full_text)], "url": ""}
#                 )
#         else:
...
#             try:
#                 full_text = post.retweeted_status["quoted_status"]["full_text"]
#                 quoted_by = post.retweeted_status["user"][
#                     "name"
#                 ]  # name of the retweet who quoted
#                 quoted_by_text = post.retweeted_status["full_text"]
#                 quoted_by_actor_username = post.retweeted_status["user"]["screen_name"]
#                 quoted_by_actor_picture = post.retweeted_status["user"][
#                     "profile_image_url"
#                 ]
#                 actor_name = post.retweeted_status["quoted_status"]["user"][
#                     "name"
#                 ]  # original tweeter info used below.
#                 actor_username = post.retweeted_status["quoted_status"]["user"][
#                     "screen_name"
#                 ]
#                 actor_picture = post.retweeted_status["quoted_status"]["user"][
#                     "profile_image_url"
#                 ]
#                 tempLikes = post.retweeted_status["quoted_status"]["favorite_count"]
#                 isQuote = True
#
#             except:  # if its not a quote default to normal retweet settings
#                 actor_name = post.retweeted_status["user"][
#                     "name"
#                 ]  # original tweeter info used below.
#                 actor_username = post.retweeted_status["user"]["screen_name"]
#                 actor_picture = post.retweeted_status["user"]["profile_image_url"]
#                 tempLikes = post.retweeted_status["favorite_count"]
#                 isRetweet = True
#             isRetweet = True
#         except:
#             isRetweet = False
...
#             try:
#                 full_text = post.quoted_status["full_text"]
#                 quoted_by = post.user["name"]  # name of the person who quoted
#                 quoted_by_text = post.full_text
#                 quoted_by_actor_username = post.user["screen_name"]
#                 quoted_by_actor_picture = post.user["profile_image_url"]
#                 actor_name = post.quoted_status["user"][
#                     "name"
#                 ]  # original tweeter info used below.
#                 actor_username = post.quoted_status["user"]["screen_name"]
#                 actor_picture = post.quoted_status["user"]["profile_image_url"]
...
#             if "entities" in post.retweeted_status["quoted_status"].keys():
#                 entities_keys = post.retweeted_status["quoted_status"][
#                     "entities"
#                 ].keys()
#                 mediaArr = post.retweeted_status["quoted_status"]["entities"].get(
#                     "media", []
#                 )
#             if "urls" in entities_keys:
#                 all_urls = post.retweeted_status["quoted_status"]["entities"]["urls"]
#         elif isQuote:  #  quote only case
#             if "entities" in post.quoted_status.keys():
#                 entities_keys = post.quoted_status["entities"].keys()
#                 mediaArr = post.quoted_status["entities"].get("media", [])
#             if "urls" in entities_keys:
#                 all_urls = post.quoted_status["entities"]["urls"]
#         elif isRetweet:
#             if "entities" in post.retweeted_status.keys():
#                 entities_keys = post.retweeted_status["entities"].keys()
#                 mediaArr = post.retweeted_status["entities"].get("media", [])
#             if "urls" in entities_keys:
#                 all_urls = post.retweeted_status["entities"]["urls"]
#         else:
#             if "entities" in tweet.keys():
#                 entities_keys = post.entities.keys()
#                 mediaArr = post.entities.get("media", [])
#             if "urls" in entities_keys:
#                 all_urls = post.entities["urls"]
#
...
#         # TODO: make sure to use and convert post.created_at to string
...
#             td.seconds, 3600
#         )  # can we scrap this and the line below ______-------________-----________---------______--------
...
#
#         finalRetweets = ""
#         tempRetweets = post.retweet_count
#         if tempRetweets <= 999:
...
#             "user_retweet": str(post.retweeted),
#             "user_fav": str(post.favorited),
#             "retweet_by": retweeted_by,
#             "quoted_by": quoted_by,
#             "quoted_by_text": quoted_by_text,
#             "quoted_by_actor_username": quoted_by_actor_username,
#             "quoted_by_actor_picture": quoted_by_actor_picture.replace(
#                 "http:", "https:"
#             ),
#         }
